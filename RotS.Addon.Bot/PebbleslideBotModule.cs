namespace RotS.Addon.Bot {
	...
	using RotS.Addon.Bot.Common;
	#endregion

	/// <summary>
	/// A Pebbleslide <seealso cref="JmcModule"/> designed for trigger spiriting in the spirit mines.
	/// </summary>
	/// <seealso cref="RotS.Addon.Core.Common.Module" />
	[ClassInterface(ClassInterfaceType.AutoDual)]
	[ComVisible(true)]
	[Guid("53C47A57-DB70-44FC-983D-1C9E0F24DB7A")]
	[ProgId("RotS.Addon.PebbleslideBotModule")]
	public class PebbleslideBotModule
		: JmcModule {
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="PebbleslideBotModule" /> class.
		/// </summary>
		/// <param name="jmcObject">The JMC object.</param>
		/// <param name="configurationFile">The configuration file.</param>
		public PebbleslideBotModule()
			: base() { }
			...
			if (enhancementSkills.Count > 0) {
				...
				enhancementSkills.ForEach(enhancementSkill => {
					...
					if (!string.IsNullOrWhiteSpace(enhancementSkillName)) {
						...
						this.Log($@"Skill registered: {skill.EnhancementSkillName}, TargetSelf: {skill.TargetSelf}", JmcColors.Cyan);
					}
				});

			}
			this.CharacterName = configuration.SafeElementValue(nameof(PebbleslideBotModule.CharacterName), string.Empty);
			this.MoveTime = configuration.SafeElementValue(nameof(PebbleslideBotModule.MoveTime), this.MoveTime);
			this.WaitTime = configuration.SafeElementValue(nameof(PebbleslideBotModule.WaitTime), this.WaitTime);
		}
		...
		protected override void OnConfigurationSettingsSaved(XElement configuration) {
			...
			configuration.Add(
				new XElement(
					nameof(PebbleslideBotModule.EnhancementSkills),
				...
				new XElement(nameof(PebbleslideBotModule.CharacterName), this.CharacterName),
				new XElement(nameof(PebbleslideBotModule.MoveTime), this.MoveTime),
				new XElement(nameof(PebbleslideBotModule.WaitTime), this.WaitTime)
				);
		}
		...
		protected override void OnIncoming(string incomingLine) {
			...
			switch (incomingLine.Trim()) {
				...
				case @"This end of the mines has come to be due to the massive barricade of":
					...
					this.Log(@"Set from line parser", JmcColors.Green);
				...
				case @"A flickering ball of pale flame dances through the air. (shadow)":
					_target = @"flame";
					this.Log($@"Target is now {_target}.", JmcColors.Green);
					break;
		...
		protected override void OnTimer(int timerID) {
			...
			switch (pebbleslideTimer) {
				...
				case PebbleslideTimer.MoveTimer:
					this.Log($@"Killing target: {_target}.", JmcColors.Red);
