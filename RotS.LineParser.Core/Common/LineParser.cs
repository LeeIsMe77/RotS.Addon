	...
	public abstract class LineParser {
		...
		public void ConfigureDatabaseConnection(string connectionString, DatabaseType databaseType) {
			...
			if (connectionStringBuilder == null) {
				this.Log(@"The database connection has been disabled.", @"red");
				return;
			}
			...
			try {
				...
				this.Log($@"A connection has been established successfully. Connection registered.", @"green");
			}
			catch (Exception caught) {
				this.Log($@"Failure Configuring Database Connection: {caught.Message}", @"red");
				throw new LineParserException(caught, $@"Failure Configuring Database Connection: {caught.Message}");
			}
		}
		...
		protected void Log(string message, string color) {
			if (this.VerboseLoggingEnabled) {
				this.JmcObject.wOutput(this.VerboseLoggingOutputWindow, message, color);
			}
		}
